...
const Page = async () => {
  ...
  const endpoints = [
    {
      ...
      example: `fetch("https://api.placeapi.site/api/payments?limit=10&page=2", {
  headers: {
    ...
    },
    {
      ...
      example: `fetch("https://api.placeapi.site/api/payments/{id}", {
  headers: {
    "X-API-KEY": "${apiKey ?? "your_api_key"}"
  }
})`,
  ...
  return (
    <div className="prose dark:prose-invert max-w-5xl">
      ...
      {/* Navigation Buttons */}
      <DocsNavigator
        prev={{ href: "/docs/cart", label: "Carts API" }}
        next={{ href: "/docs/posts", label: "Posts API" }}
      />
    </div>
  );
};
