  ...
  const endpoints = [
    {
      ...
      path: "/api/orders",
      description:
        "Retrieve a paginated list of dummy orders from the database.",
      example: `fetch("https://api.placeapi.site/api/orders?limit=10&page=2", {
  headers: {
    ...
    },
    {
      ...
      path: "/api/orders/cmehm00iq002m79fo8wkdxjhl",
      description: "Retrieve details of a single order by its ID.",
      example: `fetch("https://api.placeapi.site/api/orders/{id}", {
  headers: {
    "X-API-KEY": "${apiKey ?? "your_api_key"}"
  }
})`,
  ...
  return (
    <div className="prose dark:prose-invert max-w-5xl">
      ...
      <p className="text-lg text-gray-600 dark:text-gray-300 mb-6 leading-relaxed">
        Retrieve dummy order data from the database. Supports endpoints for
        listing all orders or fetching individual order details.
      </p>
      ...
      <div className="bg-green-100 dark:bg-gray-800 p-4 sm:p-6 rounded-lg mb-8">
        ...
        <p className="text-gray-600 dark:text-gray-300 mb-0 leading-relaxed">
          You can control the number of orders returned using{" "}
      ...
      {/* Navigation Buttons */}
      <DocsNavigator
        prev={{ href: "/docs/products", label: "Products API" }}
        next={{ href: "/docs/carts", label: "Carts API" }}
      />
    </div>
  );
};
